...
function setLocation(path) {
  ...
  locPath.hidden = false;
  locField.classList.add('picked');
  chooseFolderBtn.textContent = 'Change location';
  folderChosen();
}
...
function harnessMessage(h) {
  if (h.installed) {
    return {
      title: `${h.name} is ready`,
      parts: ['Next: what to do the moment it opens on ', { b: project.name || 'your app' }, '.'],
    };
  }
  return {
...
function renderStandard() {
  ...
  const stdPick = document.getElementById('stdPick');
  if (!h) {
    stdPick?.classList.remove('on');
    return;
  }
  ...
  const on = chosenHarness === h.id;
  stdPick.classList.toggle('on', on);
  const alert = document.getElementById('stdAlert');
  if (on) {
...
function clearFailures() {
  ...
  locPath.textContent = '';
  locPath.hidden = true;
  locField.classList.remove('picked');
  chooseFolderBtn.textContent = 'Choose location';
  document.getElementById('createApp').disabled = true;
